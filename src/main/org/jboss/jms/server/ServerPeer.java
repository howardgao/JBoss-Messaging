 ...
 *
 * @version <tt>$Revision: 7912 $</tt>
 *
 * $Id: ServerPeer.java 7912 2009-12-01 13:19:40Z gaohoward $
 */
public class ServerPeer extends ServiceMBeanSupport
{
   ...
   private SecurityStore securityStore = new SecurityMetadataStore();
   private boolean isDefaultSecurityStore = true;
   ...
   {
      if (!(securityStore instanceof SecurityMetadataStore))
      {
         throw new IllegalAccessException("SecurityStore doesn't support operation setDefaultSecurityConfig. That needs to be defined at " + securityStore.getClass().getCanonicalName() + " component");
      }
      ((SecurityMetadataStore)securityStore).setDefaultSecurityConfig(conf);
   }
   ...
   public synchronized Element getDefaultSecurityConfig()
   {
      if (!(securityStore instanceof SecurityMetadataStore))
      {
         log.warn("Operation getDefaultSecurityConfig not supported by " + securityStore.getClass().getCanonicalName());
         return null;
      }
      return ((SecurityMetadataStore)securityStore).getDefaultSecurityConfig();
   }
    ...
    public synchronized void setSecurityDomain(String securityDomain) throws Exception
   {
      if (!(securityStore instanceof SecurityMetadataStore))
      {
         throw new IllegalAccessException("SecurityStore doesn't support operation setSecurityDomain. That needs to be defined at " + securityStore.getClass().getCanonicalName() + " component");
      }
      try
      {
         ((SecurityMetadataStore)securityStore).setSecurityDomain(securityDomain);
      }
      catch (Throwable t)
      {
   ...
   public synchronized String getSecurityDomain()
   {
      ...
      {
         return ((SecurityMetadataStore)securityStore).getSecurityDomain();
      }
   }
